...
// Standard Guillontine Functions
(function() {
  "use strict";
  ...
  Guillotine = (function() {
    ...
    Guillotine.prototype._offset = function(left, top) {
      ...
      if (top || top === 0) {
        ...
        var offsetTop = (-top * 100);

        if((-top * 100) >= -150) this.canvas.style.top = offsetTop.toFixed(2) + '%';

        this.top = top;

        this.data.y = - Math.round(top * this.op.height - 75 * top);
...
$("#alterTitleImage").click(function () {
  var $picture = $(".headline-img-wrapper img");
  $picture.css("z-index", 10).css("max-height", $picture.height()).css("width", "100vw");
  $picture.guillotine({width: "100vw", height: 400});
...
$("#saveTitleImage").click(function () {
  var $picture = $(".headline-img-wrapper img");
  var $alterTitleImage = $("#alterTitleImage");
  $picture.css("z-index", -1);

  var data = $picture.guillotine('getData');
  var conf = $alterTitleImage.data();

  if(conf.url && (conf.trip || conf.company || conf.user)) {
    ...
    if(conf.trip) {
      req['trip'] = conf.trip
    } else if(conf.company) {
      req['company'] = conf.company
    } else {
      req['user'] = conf.user
    }
    ...
    $.ajax({
      ...
      success: function (res) {
        ...
        $("#modifyTrip").show();
        $alterTitleImage.show();
        $("#saveTitleImage").hide();
        location.reload(true);
      },
      error: function () {
        alert('Positioning failed! Please try again.')
      }
    });
  }
