...
from djangocms_file.models import FilerFileField as File
...
def get_episode_upload_folder(instance, pathname):
    "A standardized pathname for uploaded files and images."
    root, ext = os.path.splitext(pathname)
    return "{0}/podcasts/episodes/{1}/{2}{3}".format(settings.PODCASTING_IMG_PATH, instance.slug, slugify(root),
                                                     ext)
    # if instance.shows.count() == 1:
        
        # return "{0}/podcasts/{1}/episodes/{2}{3}".format(settings.PODCASTING_IMG_PATH, instance.shows.all()[0].slug, slugify(root), ext)
    # else:
        # return "{0}/podcasts/episodes/{1}/{2}{3}".format(settings.PODCASTING_IMG_PATH, instance.slug, slugify(root), ext)
...
class Show(models.Model):
    ...
    if 'licenses' in settings.INSTALLED_APPS:
        license = models.ForeignKey(License, verbose_name=_("license"), on_delete=models.SET_NULL, null=True)
    else:
        license = models.CharField(
            _("license"), max_length=255,
            help_text=_("To publish a podcast to iTunes it is required to set a license type."), blank=True, null=True)
    ...
    if 'photologue' in settings.INSTALLED_APPS:
        original_image = models.ForeignKey(Photo, verbose_name=_("image"), default=None, null=True, blank=True, help_text=_("""
                ...
                enclosure uploading!"""), on_delete=models.SET_NULL)
    else:
...
class Enclosure(models.Model):
    ...
    file = File(verbose_name=_('Audio Track'), blank=False, null=False, on_delete=models.CASCADE,
                help_text=_('Please upload the audio track here. Duration and size are unlimited.'))
